...
def test_per_race_softmax_temperature_sharpens():
    df = pl.DataFrame({"race_id": [1, 1, 1], "score": [1.0, 2.0, 3.0]})
    cold = per_race_softmax(df, "score", "prob", temperature=0.5)["prob"].to_numpy()
    hot = per_race_softmax(df, "score", "prob", temperature=2.0)["prob"].to_numpy()
...
def test_per_race_softmax_temperature_one_matches_default():
    df = pl.DataFrame({"race_id": [1, 1, 1], "score": [1.0, 2.0, 3.0]})
    default = per_race_softmax(df, "score", "prob")["prob"].to_numpy()
    explicit = per_race_softmax(df, "score", "prob", temperature=1)["prob"].to_numpy()
